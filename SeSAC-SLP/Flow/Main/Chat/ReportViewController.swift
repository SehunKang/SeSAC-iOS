...
//  SeSAC-SLP
//
//  Created by Sehun Kang on 2022/02/18.
//
...
class ReportViewController: UIViewController {
    ...
    let placeholderSting = "신고 사유를 적어주세요\n허위 신고 시 제재를 받을 수 있습니다."
    ...
    private let argue: UIButton = {
        let button = UIButton()
        button.setTitle("불편한 언행", for: .normal)
        button.reportConfigure()
        button.addTarget(self, action: #selector(buttonClicked(_:)), for: .touchUpInside)
        button.tag = 1
        return button
    ...
    private func doneButtonClicked() {
        let data: [String: Any] = ["otheruid": uid, "reportedReputation": result, "comment": textView.text!]
...
extension UIButton {
    ...
    func reportConfigure() {
        titleLabel?.font = CustomFont.Title4_R14.font
        setTitleColor(CustomColor.SLPBlack.color, for: .normal)
        backgroundColor = .clear
        layer.borderColor = CustomColor.SLPGray4.color.cgColor
        layer.borderWidth = 1
        layer.cornerRadius = 8
    }
    
    func reportButtonSelected() {
        setTitleColor(CustomColor.SLPWhite.color, for: .selected)
